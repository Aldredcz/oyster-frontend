...
class NotificationsStore {
	...
	@action fetchNotifications (): ?Promise<Array<string>> {
		if (!moduleManager.isLoggedIn) {
			return
		}
		return NotificationAPI.fetchAll()
			.then(
				(notifications) => notifications.map((notification) => {
					this.notificationsMap.set(notification.uuid, notification)
	...
	@computed get notifications () {
		return [...this.notificationsMap.values()].sort((a, b) => compareDatesDesc(a.createdAt, b.createdAt))
	}
