...
export default class MoveDetector extends React.Component<TProps> {
	static defaultProps = {
		touchEvents: true,
		scrollEvents: true,
		dragEvents: true,
	...
	handleTouchStart = (ev: React.TouchEvent<HTMLDivElement>) => {
		const {touches} = ev
		if (touches.length === 1) {
	...
	handleTouchMove = throttle((ev: React.TouchEvent<HTMLDivElement>) => {
		ev.preventDefault()
		ev.stopPropagation()
		...
		if (touches.length === 1) {
			...
			this.props.onMove({dX, dY, event: ev as $FixMe, type: 'touch'})
		}
	}, 16, {trailing: false})

	handleWheel = (ev: React.WheelEvent<HTMLDivElement>) => {
		let dY = ev.deltaY
		let dX = ev.deltaX
		...
		this.props.onMove({dX, dY, event: ev as $FixMe, type: 'scroll'})
	}
